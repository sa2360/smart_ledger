#!/bin/bash
# ============================================================
# 一语记 · 中转服务一键部署（阿里云 Ubuntu 22.04）
# 用法：在本地 Git Bash 里执行
#   ssh root@8.163.98.62 'bash -s' < deploy-aliyun.sh
# ============================================================
set -e

echo "==> [1/5] 安装 Node.js 20（NodeSource 源）"
if ! command -v node >/dev/null 2>&1 || [ "$(node -v | cut -c2-3 | cut -d. -f1)" -lt 18 ]; then
  curl -fsSL https://deb.nodesource.com/setup_20.x | bash -
  apt-get install -y nodejs
fi
node -v

echo "==> [2/5] 写入中转服务 /opt/yiyuji/relay.js"
mkdir -p /opt/yiyuji
cat > /opt/yiyuji/relay.js <<'RELAY_EOF'
// ============================================================
...
RELAY_EOF
node --check /opt/yiyuji/relay.js && echo "relay.js 语法检查 OK"

echo "==> [3/5] 写入 systemd 服务（开机自启 + 崩溃自动拉起）"
cat > /etc/systemd/system/yiyuji-relay.service <<'UNIT_EOF'
[Unit]
Description=Yiyuji AI Relay
After=network.target

[Service]
ExecStart=/usr/bin/node /opt/yiyuji/relay.js
Environment=PORT=8000
Environment=AGNES_KEY=sk-C4EIUQ3wdrKgW34FZhm4BoA0VqDbBBM9rmNq2BZwEYRHrYwe
Environment=BAIDU_API_KEY=nXvUpSZtmhTZ9i8nVfBzHJhg
Environment=BAIDU_SECRET_KEY=qOhP1Zsw47qcuXqFBjFbz50pEgCmSG4C
Restart=always
RestartSec=3
User=root
...
systemctl enable --now yiyuji-relay
sleep 2
systemctl status yiyuji-relay --no-pager | head -6 || true

echo "==> [5/5] 本机自检"
curl -s http://127.0.0.1:8000/health && echo
echo ""
echo "========== 部署完成 =========="
echo "外网验证：浏览器或 App 访问 http://8.163.98.62:8000/health"
echo "如果外网不通，检查阿里云安全组是否放行了 TCP 8000"
